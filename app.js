...
document.getElementById('loginForm').onsubmit = async function(e) {
    ...
    try {
        const response = await apiRequest('http://localhost:3000/api/login', 'POST', { username, password });
        if (response.error) {
...
document.getElementById('signupForm').onsubmit = async function(e) {
    ...
    try {
        const response = await apiRequest('http://localhost:3000/api/signup', 'POST', { username, password, isAdmin });
        if (response.error) {
            alert(response.error);
            return;
        }
...
async function updateStatusDisplay() {
    try {
        const response = await apiRequest('http://localhost:3000/api/status', 'GET');
...
document.getElementById('saveStatus').onclick = async () => {
    ...
    try {
        await apiRequest('http://localhost:3000/api/status', 'POST', { status: newStatus }, { Authorization: `Bearer ${apiKey}` });
        updateStatusDisplay();
    } catch (error) {
...
async function updateChangelogsDisplay() {
    try {
        const changelogs = await apiRequest('http://localhost:3000/api/changelogs', 'GET');
...
document.getElementById('addChangelog').onclick = async () => {
    ...
    if (newLog) {
        try {
            await apiRequest('http://localhost:3000/api/changelogs', 'POST', { log: newLog }, { Authorization: `Bearer ${apiKey}` });
...
async function loadConfig() {
    try {
        const config = await apiRequest('http://localhost:3000/api/config', 'GET', null, { Authorization: `Bearer ${apiKey}` });
...
document.getElementById('configForm').onsubmit = async function(e) {
    ...
    try {
        await apiRequest('http://localhost:3000/api/config', 'POST', config, { Authorization: `Bearer ${apiKey}` });
